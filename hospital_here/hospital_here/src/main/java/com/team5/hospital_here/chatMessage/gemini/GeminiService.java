...
public class GeminiService {
    ...
    public ResponseDTO prompt(GeminiRequestDTO geminiRequestDTO) {
        ...
        //department db 에서 불러와서 메시지 형식 지정
        String departmentNames = departments.stream()
            .map(Department::getName)
            .reduce((a, b) -> a + "','" + b)
            .orElse("");

        String prompt = "'" + geminiRequestDTO.getContents().getParts().getText() +
            "'에 대한 소견을 [" + departmentNames
            + "]에서 최대 3개를 포함해서 부탁해. 선택된 3개의 진료과 단어는 오직 느낌표쌍(!!)만을 앞뒤로 감싸서 표시해줘 (ex. !!내과!!)";

        GeminiRequestDTO updatedRequestDTO = new GeminiRequestDTO(prompt);

        GeminiResponseDTO response;
        try {
            response = restTemplate.postForObject(url + "?key=" + key, updatedRequestDTO,
        ...
        //증상 저장
        saveMessage(geminiRequestDTO.getContents().getParts().getText());
